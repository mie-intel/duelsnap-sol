...
function useFaucetCooldown(address: string | undefined) {
  ...
  useEffect(() => {
    const tick = () =>
      setRemaining(Math.max(0, cooldownUntilRef.current - Date.now()));
    tick();
...
export default function ProfilePage() {
  ...
  const refreshBalances = useCallback(async () => {
    ...
    const [lamports, token] = await Promise.all([
      ...
      connection.getTokenAccountBalance(paymentAta(publicKey)).catch(() => null),
    ]);
  ...
  const handleFaucet = async () => {
    if (!address || faucet.onCooldown) return;
    ...
    try {
      ...
      if (!res.ok) {
        if (data.cooldownUntil) faucet.setClaimed();
        setFaucetMsg(data.error ?? "Claim failed");
      } else {
        faucet.setClaimed();
        setFaucetMsg("+3.00 USDC received");
  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 pt-6 pb-24 bg-bg-page">
      ...
      <button
        ...
        disabled={faucetLoading || faucet.onCooldown}
        className={`w-full py-3 rounded-2xl font-semibold text-xs font-sans transition-all border mb-1 ${
          faucet.onCooldown
      ...
      >
        {faucetLoading
          ? "Claiming..."
          : faucet.onCooldown
            ? formatCountdown(faucet.remaining)
            : "Claim test USDC"}
      </button>
